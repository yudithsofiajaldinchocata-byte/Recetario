...
const rutaUploadsEstaticos = path.join(process.cwd(), 'uploads');
const rutaUploadsRecetas = path.join(rutaUploadsEstaticos, 'recetas');
if (!fs.existsSync(rutaUploadsRecetas)) {
  fs.mkdirSync(rutaUploadsRecetas, { recursive: true });
}
app.use('/uploads', express.static(rutaUploadsEstaticos));
